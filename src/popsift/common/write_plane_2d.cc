...
void write_plane2Dppm( const char* filename, Plane2D_float& f, const std::vector<int2>& red )
{
    ...
    bool* is_red = new bool[rows * cols];
    for( int i=0; i<rows*cols; i++ ) is_red[i] = false;

    for( auto pixel : red )
    {
        if( pixel.y >= rows )
            POP_FATAL( "Programming error, red pixel is out of range in " << __FUNCTION__ );
        if( pixel.x >= cols )
            POP_FATAL( "Programming error, red pixel is out of range in " << __FUNCTION__ );
        is_red[pixel.y * cols + pixel.x] = true;
    }
